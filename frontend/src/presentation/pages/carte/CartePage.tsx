...
export function CartePage() {
  ...
  const selectedParcelle = useMemo(
    () => parcelles.find((p) => p.id === selected) ?? null,
    [parcelles, selected],
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        <div className="lg:col-span-3 bg-card border border-border rounded-xl overflow-hidden">
          ...
          <div className={`relative bg-gradient-to-br from-success/5 via-info/5 to-primary/10 overflow-hidden ${mode === '3d' ? 'h-[min(72vh,720px)] min-h-[480px]' : 'aspect-[4/3]'}`}>
            {mode === '3d' && (
              <CesiumMap
                parcelles={parcelles}
                selectedId={selected}
        ...
        <aside className="bg-card border border-border rounded-xl flex flex-col max-h-[720px]">
          ...
          {selectedParcelle && mode === '3d' && (
            <div className="p-3 border-b border-border bg-secondary/40 space-y-1.5">
              <div className="text-[10px] uppercase tracking-wide text-muted-foreground">
                Sélection · {selectedParcelle.type === 'maison' ? 'Maison' : 'Terrain'}
              </div>
              <div className="text-sm font-semibold leading-snug">{selectedParcelle.titre}</div>
              <div className="text-xs text-muted-foreground">{selectedParcelle.quartier}, {selectedParcelle.ville}</div>
              <div className="flex items-center gap-2 flex-wrap">
                <StatusBadge statut={selectedParcelle.statut} />
                <span className="text-xs font-display font-bold text-primary">{xaf(selectedParcelle.prix)}</span>
              </div>
              {selectedParcelle.type === 'maison' && (
                <div className="text-[11px] text-muted-foreground pt-1">
                  {[
                    selectedParcelle.typeBien,
                    selectedParcelle.surface_m2 != null ? `${selectedParcelle.surface_m2} m²` : null,
                    selectedParcelle.chambres != null ? `${selectedParcelle.chambres} ch.` : null,
                    selectedParcelle.etages != null ? `${selectedParcelle.etages} ét.` : null,
                  ].filter(Boolean).join(' · ')}
                </div>
              )}
            </div>
            ...
            {markers.map((m) => (
              <button key={m.id} onClick={() => setSelected(m.id)}
                ...
                <div className="flex items-center justify-between mt-0.5 gap-2">
                  <div className="text-xs font-medium text-primary">{xaf(m.prix)}</div>
                  <StatusBadge statut={m.statut} />
                </div>
              </button>
            ))}
