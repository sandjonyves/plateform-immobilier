...
import { Plus, Search, Receipt, ArrowUpRight, FileDown } from 'lucide-react';
...
export function VentesPage() {
  const { ventes, charger } = useVenteStore();
  ...
  const filtered = useMemo(() => ventes.filter(t =>
    (bienType === 'tous' || t.bien_type === bienType) &&
    (statut === 'tous' || t.statut === statut) &&
    (q === '' || t.id.toLowerCase().includes(q.toLowerCase()) || t.bien_id.toLowerCase().includes(q.toLowerCase())),
  ), [ventes, q, bienType, statut]);

  const kpi = useMemo(() => {
    const conf = ventes.filter(t => t.statut === 'confirmee');
    return {
      total: ventes.length,
      montant: conf.reduce((s, t) => s + t.montant, 0),
      terrains: conf.filter(t => t.bien_type === 'terrain').reduce((s, t) => s + t.montant, 0),
      attente: ventes.filter(t => t.statut === 'en_attente').length,
    };
  }, [ventes]);
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        <KpiCard label="Ventes" value={kpi.total.toString()} delta={4.6} accent="primary" />
        <KpiCard label="Confirmées" value={xaf(kpi.montant)} delta={12.5} accent="success" />
        <KpiCard label="Terrains confirmés" value={xaf(kpi.terrains)} delta={3.8} accent="info" />
      ...
      <div className="bg-card border border-border rounded-xl">
        <div className="p-4 border-b border-border flex flex-wrap items-center gap-2">
          ...
          <select value={bienType} onChange={(e) => setBienType(e.target.value)} className="h-9 px-3 text-sm rounded-lg border border-border bg-background">
            ...
            <tbody>
              {filtered.map((t) => (
                <tr key={t.id} className="border-b border-border last:border-0 hover:bg-secondary/40">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="text-xs inline-flex items-center gap-1">
                          ...
                          {t.bien_type === 'terrain' ? 'Terrain' : 'Maison'}
                        </div>
                  ...
                  <td className="px-4 py-3 text-xs text-muted-foreground">{format(new Date(t.date_vente), 'dd/MM/yyyy')}</td>
                </tr>
              ))}
              {filtered.length === 0 && <tr><td colSpan={7} className="px-4 py-10 text-center text-sm text-muted-foreground">Aucune vente.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
    </div>
    <VenteForm open={openForm} onClose={() => setOpenForm(false)} />
    </>
  );
}
